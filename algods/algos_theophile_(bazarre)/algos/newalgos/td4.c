...
void InsertValue(BinaryHeap *heap, int value)
{
    ...
    for (int i = heap->filled - 1; heap->array[i] > heap->array[(i-1)/2] && i > 0;) {
...
int ExtractMax(BinaryHeap * heap, int *res)
{
    ...
    *res = heap->array[0];
    ...
    while (1) {
        ...
        int right =  2*i + 2;
        int largest = i;
        if (left < heap->filled && heap->array[left] > heap->array[largest]) largest = left;
        if (right < heap->filled && heap->array[right] > heap->array[largest]) largest = right;
        if (largest == i ) break;
        int tmp = heap->array[largest];
