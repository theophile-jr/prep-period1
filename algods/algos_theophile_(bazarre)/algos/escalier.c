...
int main(void) {
    ...
    int dp[nbMarche + 1];
    for (i = 0; i < nbMarche; i++) dp[i] = 0;
    dp[0] = 1;

    for (i = 1; i <= nbMarche ; i++) {
        dp[i] = dp[i - 1];
        if (i - 2 >= 0 && positionPaliers[i-1] != 1) {
            dp[i] += dp[i - 2];
        }
// si pas palier avant : on vient de la case avant ou bien de celle avant avant, sinon on a pas pu doubler sauter
    }
    printf("%d\r\n", dp[nbMarche]);
